...
class MailNotificationWizard(models.Model):
    _name = 'mail.notification.wizard'
    _description = 'Mail Notification Wizard'
        ...
        if self.admission_ids:
            for admission_id in self.admission_ids:
                admission_id.sudo().write({
                    'document_mssg_subject': self.document_mssg_subject,
                    'document_mssg_body': self.document_mssg_body,
                })
            template_id = self.env.ref('vg_colg.mail_notification_template').sudo()
